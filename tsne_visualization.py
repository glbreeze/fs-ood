...
def subset_torch_data(dataset, max_count, num_classes=None):
    indices = []
    ...
    if num_classes is not None and num_classes<=len(class_to_indices.keys()):
        selected_classes = random.sample(list(class_to_indices), num_classes)
    else:
    ...
    subset_loader = torch.utils.data.DataLoader(dataset, batch_size=32, shuffle=False)
...
def main(args):
    ...
    id_loader = torch.utils.data.DataLoader(
            subset_torch_data(id_loader.dataset, max_count=100, num_classes=10),
            batch_size=args.batch_size, shuffle=False)
    
    # ==== OD data 
    out_dataset = 'iNaturalist'
    od_loader = set_ood_loader_ImageNet(args, out_dataset, preprocess)
    od_loader = torch.utils.data.DataLoader(
            subset_torch_data(od_loader.dataset, max_count=100),
            batch_size=args.batch_size, shuffle=False)
