...
class TwoCropTransform:
    """Apply two augmentations: global crop and local crop."""
    
    def __init__(self, global_transform, local_transform):
        """
        Args:
            global_transform (Callable): Transform for global crop.
            local_transform (Callable): Transform for local crop.
        """
    ...
    def __call__(self, img):
        ...
        Returns:
            Tuple: (global_crop, local_crop)
        """
        global_crop = self.global_transform(img)
        local_crop = self.local_transform(img)
...
def _build_transform_train(cfg, choices, target_size, normalize):
    ...
    if "two_crop" in choices:
        tfm_train_g, tfm_train_l = [], []
        if "random_resized_crop" in choices:
            print(f"+ random resized crop (size={input_size}, global scale={cfg.INPUT.GLOBAL_RRCROP_SCALE}, local scale={cfg.INPUT.LOCAL_RRCROP_SCALE})")
            tfm_train_g += [RandomResizedCrop(input_size, scale=cfg.INPUT.GLOBAL_RRCROP_SCALE, interpolation=interp_mode)]
            tfm_train_l += [RandomResizedCrop(input_size, scale=cfg.INPUT.LOCAL_RRCROP_SCALE, interpolation=interp_mode)]

        if "random_flip" in choices:
            print("+ random flip")
            tfm_train_g += [RandomHorizontalFlip()]
            tfm_train_l += [RandomHorizontalFlip()]
        ...
        if "colorjitter" in choices:
            ...
            print("+ color jitter (brightness={b_}, contrast={c_}, saturation={s_}, hue={h_})")
            tfm_train_g += [ColorJitter(brightness=b_, contrast=c_, saturation=s_, hue=h_,)]
            tfm_train_l += [ColorJitter(brightness=b_, contrast=c_, saturation=s_, hue=h_,)]

        if "randomgrayscale" in choices:
            print("+ random gray scale")
            tfm_train_g += [RandomGrayscale(p=cfg.INPUT.RGS_P)]
            tfm_train_l += [RandomGrayscale(p=cfg.INPUT.RGS_P)]

        if "gaussian_blur" in choices:
            print(f"+ gaussian blur (kernel={cfg.INPUT.GB_K})")
            gb_k, gb_p = cfg.INPUT.GB_K, cfg.INPUT.GB_P
            tfm_train_g += [RandomApply([GaussianBlur(gb_k)], p=gb_p)]
            tfm_train_l += [RandomApply([GaussianBlur(gb_k)], p=gb_p)]

        print("+ to torch tensor of range [0, 1]")
        tfm_train_g += [ToTensor()]
        tfm_train_l += [ToTensor()]

        if "normalize" in choices:
            print(f"+ normalization (mean={cfg.INPUT.PIXEL_MEAN}, std={cfg.INPUT.PIXEL_STD})")
            tfm_train_g += [normalize]
            tfm_train_l += [normalize]

        if "gaussian_noise" in choices:
            print(f"+ gaussian noise (mean={cfg.INPUT.GN_MEAN}, std={cfg.INPUT.GN_STD})")
            tfm_train_g += [GaussianNoise(cfg.INPUT.GN_MEAN, cfg.INPUT.GN_STD)]
            tfm_train_l += [GaussianNoise(cfg.INPUT.GN_MEAN, cfg.INPUT.GN_STD)]

        tfm_train = TwoCropTransform(Compose(tfm_train_g), Compose(tfm_train_l))
