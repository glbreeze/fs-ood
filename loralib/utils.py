...
def apply_lora(cfg, vision_encoder):
    # vision_encoder = clip_model.visual.transformer
    params = ['q', 'k', 'v']
    list_lora_layers = []

    indices = INDEX_POSITIONS_VISION[cfg.MODEL.BACKBONE.NAME][cfg.TRAINER.ADAPTERS.LORA_POSITION]
            ...
            for name, submodule in block.named_children():
                if isinstance(submodule, nn.MultiheadAttention):
                    new_multi_head_lora = PlainMultiheadAttentionLoRA(
                        submodule, enable_lora=params, r=cfg.TRAINER.ADAPTERS.LORA_R, lora_alpha=cfg.TRAINER.ADAPTERS.LORA_A, dropout_rate=cfg.TRAINER.ADAPTERS.LORA_DR)
